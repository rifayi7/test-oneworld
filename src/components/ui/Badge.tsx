...
export function Badge({ children, icon, bgColor = "white", className = "" }: BadgeProps) {
  ...
  const icons = {
    squeegee: <Brush className="w-4 h-4 text-primary-600 stroke-[2.5]" />,
    bug: <Bug className="w-4 h-4 text-slate-800" />,
    wrench: <Wrench className="w-4 h-4 text-slate-800" />,
    bolt: <Zap className="w-4 h-4 text-primary-600 fill-current" />,
    drop: <Droplet className="w-4 h-4 text-primary-600 stroke-[2.5]" />,
  ...
  const bgStyles = {
    ...
    blue: "bg-white border-primary-100 shadow-md text-primary-900",
  ...
  return (
    <div
      className={`inline-flex items-center gap-2 border px-4.5 py-2.5 rounded-badge text-xs font-bold select-none whitespace-nowrap ${bgStyles[bgColor]} ${className}`}
    >
